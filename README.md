...
Sistema web completo para candidatos preencherem a ficha de cadastro pelo celular ou computador, e um painel administrativo onde o RH acompanha os cadastros e gera a ficha em PDF.

## O que o sistema faz
- **Cadastro público** (`index.html`): ficha em 6 etapas com todos os campos da ficha original, cálculo automático de idade, busca de endereço por CEP, máscaras de CPF/telefone/data, foto do candidato e validações.
- **Confirmação com protocolo**: ao finalizar, o candidato recebe um número de protocolo. A geração de PDF **não** fica disponível para o candidato — apenas o RH gera a ficha.
- **Painel administrativo** (`admin.html`): login protegido, painel com totais, busca, filtros por status e data, visualização completa de cada cadastro, alteração de status, preenchimento dos campos do departamento pessoal, **geração da ficha em PDF** e exclusão.

## Estrutura das pastas
```
vegas/
├── index.html    (cadastro público — precisa ficar na raiz)
...
└── backend/      → Google Apps Script + instruções (não faz parte do site)
    ├── Code.gs
    └── INSTRUCOES-BACKEND.md
```

## Como testar agora (modo demonstração)
Não precisa instalar nada. Abra `index.html` num navegador (de preferência servindo a pasta, veja abaixo) e faça um cadastro. Nesse modo os dados ficam salvos **no próprio navegador**, só para você experimentar.

- Painel de teste: abra `admin.html` e entre com **usuário `admin`** e **senha `Vegas4747@`**.
...
3. Cole a URL gerada no arquivo `config.js` (linha `API_URL`).
...
## Identidade visual
As cores e a marca seguem o logotipo da Vegas: preto, prata e cinza. Para trocar textos, cores ou o logotipo, os arquivos são `styles.css` (cores/estilos) e `config.js` (nome, status, empresas do grupo).
